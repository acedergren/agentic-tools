...
| "Terraform should add ZPR attributes to production resources." | Route to `infrastructure-as-code` plus `oci/zpr-security`; require policy-first sequencing, canary rollout, imports, and rollback. |
| "Terraform should create a Bastion for private instance access." | Route to `infrastructure-as-code` plus `oci/managed-bastion-access`; avoid public SSH, protect keys/state, and scope allowlists/IAM. |
